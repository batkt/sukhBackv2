...
async function sync() {
  const uris = [
    ...
    "mongodb://admin:Br1stelback1@192.168.1.88:27017/bpaySukh?authSource=admin"
  ].filter(Boolean);
  ...
  try {
    ...
    // We need to mock the 'kholbolt' object for the model function
    const mockKholbolt = { dbName: mongoose.connection.name };
    ...
    console.log(`✅ Found record: ${qpayObject._id}. Status: ${qpayObject.tulsunEsekh}`);
