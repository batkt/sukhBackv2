...
async function fixOrphanData() {
  ...
  try {
    ...
    for (const kholbolt of kholboltuud) {
      ...
      console.log(`\n[MIGRATION] Processing organization: ${baiguullagiinId}`);
      ...
      try {
        ...
        try {
          ...
          const mashinDocs = await mashinModel(kholbolt.kholbolt).find({}).lean();
        ...
        try {
          ...
          const mashinDocs = await MashinUpper(kholbolt.kholbolt).find({}).lean();
          for (const m of mashinDocs) {
        ...
        const UilchluulegchModel = Uilchluulegch(kholbolt.kholbolt);
        ...
        for (const { name, label } of collectionsToFix) {
          try {
            const Model = name 
              ? Uilchluulegch(kholbolt.kholbolt, false, name)
              : Uilchluulegch(kholbolt.kholbolt);
            
            // Find records missing turul but having mashiniiDugaar
            const orphanRecords = await Model.find({
              $or: [
                { turul: { $exists: false } },
                { turul: null },
                { turul: "" }
        ...
        console.log(`  Error processing organization: ${e.message}`);
      }
    }
